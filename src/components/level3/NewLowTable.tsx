          ...
          <table className="w-full">
            <thead className="sticky top-0 z-10">
              <tr className="font-semibold tracking-wider uppercase text-[var(--text-tertiary)] bg-[var(--bg-surface)] border-b border-[var(--border-default)]" style={{ fontSize: 'var(--text-label)' }}>
                ...
                <th className="text-right py-3 pr-5 hidden lg:table-cell">거래량</th>
              </tr>
            </thead>
            <tbody>
              {sorted.slice(0, 50).map((s) => (
                <tr key={s.ticker}
                  ...
                  <td className="py-3 text-right text-[var(--text-muted)] font-mono pr-5 hidden lg:table-cell" style={{ fontSize: 'var(--text-body)' }}>
                    {fmtVolume(s.volume)}
                  </td>
                </tr>
              ))}
