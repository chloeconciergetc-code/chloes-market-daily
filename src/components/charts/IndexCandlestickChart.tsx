    ...
    <div>
      <div className="flex items-baseline gap-2 mb-3 flex-wrap">
        ...
        <button onClick={() => setShowMA(v => !v)}
          className={`ml-auto px-2 py-0.5 font-semibold tracking-wide uppercase rounded-[var(--radius-xs)] transition-all ${
            showMA ? 'bg-[var(--bg-elevated)] text-[var(--text-secondary)]' : 'text-[var(--text-muted)] hover:bg-[var(--bg-subtle)]'
          }`}
          style={{ fontSize: 'var(--text-micro)' }}>
          MA
        </button>
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" preserveAspectRatio="xMidYMid meet">
        {yTicks.map((val, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={width - margin.right + 4} y={y + 3.5}
        ...
        {candles.map((c, i) => {
          ...
          return (
            <g key={c.d}>
              <line x1={x} x2={x} y1={yScale(c.h)} y2={yScale(c.l)} stroke={color} strokeWidth={0.7} opacity={0.6} />
              <rect x={x - candleW / 2} y={bodyTop} width={candleW} height={bodyH}
                fill={up ? 'transparent' : color} stroke={color} strokeWidth={0.7} rx={0.5} />
            </g>
          )
        })}
        ...
        <g>
          <rect x={width - margin.right + 1} y={yScale(lastCandle.c) - 8} width={48} height={16} rx={3}
            fill={isUp ? 'var(--color-up)' : 'var(--color-down)'} />
          <text x={width - margin.right + 25} y={yScale(lastCandle.c) + 3}
            fill="white" fontSize={8} textAnchor="middle" fontFamily="var(--font-mono)" fontWeight="600">
          ...
          <line x1={xScale(candles.length - 1)} x2={width - margin.right + 1}
            ...
            strokeWidth={0.5} strokeDasharray="2 2" opacity={0.4} />
        </g>

        {showMA && (
          <g transform={`translate(${margin.left + 2}, ${margin.top - 6})`}>
            <rect x={-2} y={-6} width={96} height={14} rx={3} fill="rgba(0,0,0,0.4)" />
            <line x1={2} x2={12} y1={0} y2={0} stroke="var(--color-ma20)" strokeWidth={1.2} strokeLinecap="round" />
            <text x={16} y={3} fill="var(--text-muted)" fontSize={8} fontFamily="var(--font-mono)">MA20</text>
            <line x1={48} x2={58} y1={0} y2={0} stroke="var(--color-ma60)" strokeWidth={1.2} strokeLinecap="round" />
            <text x={62} y={3} fill="var(--text-muted)" fontSize={8} fontFamily="var(--font-mono)">MA60</text>
          </g>
        )}
      </svg>
    </div>
