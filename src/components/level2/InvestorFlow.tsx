    ...
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full" preserveAspectRatio="xMidYMid meet">
      ...
      {data.map((d, i) => {
        ...
        const drawBar = (val: number, offset: number, color: string) => {
          ...
          return <rect key={offset} x={x + offset * (barW + 2)} y={y} width={barW} height={Math.max(h, 1)} fill={color} opacity={0.75} rx={2} />
        }
  ...
  return (
    <div className="grid grid-cols-3 gap-3 mb-4">
      {[
        ...
        <div key={label} className="bg-[var(--bg-elevated)] rounded-[var(--radius-md)] p-3 text-center">
          <div className="text-[var(--text-tertiary)] uppercase tracking-wider mb-1" style={{ fontSize: 'var(--text-caption)' }}>{label}</div>
          <div className={`font-mono font-bold ${cls(today)}`} style={{ fontSize: 'var(--text-title)' }}>{fmt(today)}</div>
          <div className="text-[var(--text-tertiary)] mt-0.5" style={{ fontSize: 'var(--text-caption)' }}>
            5일 <span className={`font-mono font-semibold ${cls(cum)}`}>{fmt(cum)}</span>
          </div>
        </div>
      ))}
    </div>
